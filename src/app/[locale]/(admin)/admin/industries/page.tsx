...
export default function AdminIndustriesPage() {
  ...
  const [data, setData] = useState<{ items: Industry[]; total: number } | null>(
    null
  );
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Define industries used across services and portfolios.
          </p>
        </div>
        <Button onClick={() => setIsCreateOpen(true)}>Create industry</Button>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow>
                <TableCell colSpan={5}>No industries found.</TableCell>
              </TableRow>
            )}
          </TableBody>
        </Table>
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 w-[92vw] max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-xl border bg-white p-5 shadow-2xl">
            ...
            <Dialog.Description className="mt-1 text-sm text-muted-foreground">
              ...
              <span className="font-semibold">
                {pendingDelete?.name_i18n?.vi || "this industry"}
              </span>
              ?
            </Dialog.Description>
            <div className="mt-5 flex items-center justify-end gap-2">
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  try {
                    await deleteAdminIndustry(pendingDelete._id);
            ...
            <div className="flex items-center justify-between border-b border-slate-200 bg-white px-6 py-4">
              <div>
                ...
                <Dialog.Description className="text-xs text-muted-foreground">
                  Add a new industry for tagging services and portfolios.
                </Dialog.Description>
              </div>
              <Dialog.Close asChild>
          ...
          >
            <div className="flex items-center justify-between border-b border-slate-200 bg-white px-6 py-4">
              <div>
                ...
                <Dialog.Description className="text-xs text-muted-foreground">
                  Update industry details and visibility.
                </Dialog.Description>
              </div>
              <Dialog.Close asChild>
                <Button variant="outline" className="rounded-full">
